...
def load_csv(file, column_names):
    ...
    df["uid"] = (df["video_id"] + ":" + df["frame_id"].map(str) + ":" + df["track_id"])
...
def run_evaluation(groundtruth, predictions, verbose=False, threshold=0.5):
    ...
    df_groundtruth = load_csv(
        groundtruth,
        column_names=[
            "video_id", "frame_id", "entity_box_x1", "entity_box_y1",
            "entity_box_x2", "entity_box_y2", "track_id", "label"
        ])
    df_predictions = load_csv(
        predictions,
        column_names=[
            "video_id", "frame_id", "entity_box_x1", "entity_box_y1",
            "entity_box_x2", "entity_box_y2", "track_id", "label", "score"
        ])
